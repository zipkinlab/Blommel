...
constants <- list(nG = nG, v = v, B = B, mdpt = mdpt, nobs = sum(spec==6),
                  nstart = nstart, nend = nend, nsites = nsites,
                  site = site[spec == 6], offset = offset, region = region,
                  migration = migration)

data <- list(y = y[,,6], dclass = dclass[spec == 6])
...
ID <- paste("spec6_chain", length(list.files(pattern = "spec6_chain", full.names = FALSE)) + 1, sep="")
assign(ID, out)
