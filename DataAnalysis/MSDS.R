...
constants <- list(nG = nG, v = v, B = B, mdpt = mdpt, nobs = sum(spec==10),
                  nstart = nstart, nend = nend, nsites = nsites,
                  site = site[spec == 10], offset = offset, region = region,
                  migration = migration)

data <- list(y = y[,,10], dclass = dclass[spec == 10])
...
ID <- paste("spec10_chain", length(list.files(pattern = "spec10_chain", full.names = FALSE)) + 1, sep="")
assign(ID, out)
