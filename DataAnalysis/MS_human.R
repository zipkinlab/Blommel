...
data <- list(nD = nD, v = v, site = site[spec < 15], rep = rep[spec < 15], spec = spec[spec < 15],
             ...
             nreps = nreps, gs = gs[spec < 15], Migrant = Migrant, Cattle = Cattle, Shoat = Shoat, border = boundDst, offset = offset, region = region)
...
out1 <- jagsUI(data = data, inits = inits, parameters.to.save = params, model.file = "MS_human.txt", 
